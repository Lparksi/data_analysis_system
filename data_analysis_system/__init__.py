...
def compute_excel(file, template_option, config_file):
    ...
    if 'max_source' in config.sections():
        # Iterate over options in the [max_source] section
    ...
    if 'config' in config.sections():
        # Iterate over options in the [user] section
    ...
    result_df.to_excel(os.path.join("uploads", ret_file_name), index=False)
